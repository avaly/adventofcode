...
describe('generators', () => {
	test('all combinations', () => {
		const gen = generateAllCombinations(['a', 'b', 'c', 'd'], 2);
		...
		for (const combo of generateAllCombinations(['a', 'b', 'c'], 2, [])) {
			result.push(combo);
		}
	...
	test('unique combinations', () => {
		const gen = generateUniqueCombinations(['a', 'b', 'c', 'd'], 2);
		...
		for (const combo of generateUniqueCombinations(['a', 'b', 'c'], 2, [])) {
			result.push(combo);
		}
