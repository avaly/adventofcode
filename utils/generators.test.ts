...
describe('generators', () => {
	...
	test('all combinations with custom validation', () => {
		const gen = generateAllCombinations(
			['a', 'b', 'c', 'd'],
			[2, 2],
			[],
			(partial) => partial[0] !== 'b',
	...
	test('unique combinations with custom validation', () => {
		const gen = generateUniqueCombinations(
			['a', 'b', 'c', 'd'],
			[2, 2],
			[],
			(partial) => partial[0] !== 'b',
		...
		for (const combo of generateUniqueCombinations(
			['a', 'b', 'c', 'd'],
			[2, 3],
			[],
			(partial) => partial[0] !== 'b',
